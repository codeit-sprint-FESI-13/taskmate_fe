...
export const TodoList = ({ goalId, keyword, isMyTodo }: TodoListProps) => {
  const [selectedSort, setSelectedSort] =
    useState<keyof typeof SORT_OPTIONS>("마감일 순");
  ...
  const queryOptions = useMemo(
    () => ({
      ...
      queryFn: async ({ pageParam }: { pageParam: TodoCursor }) => {
        const response = await todoApi.getTodoList(goalId, {
          sort,
          mineOnly: isMyTodo,
      ...
      getNextPageParam: (
        lastPage: Awaited<ReturnType<typeof todoApi.getTodoList>>["data"],
      ) => {
  ...
  return (
    <div className="h-full w-full">
      <div className="flex w-full items-center justify-between">
        <h3 className="typography-body-1 font-bold">TODO</h3>
        <Order
      ...
      <ul className="relative h-[728px] w-full overflow-y-scroll rounded-4xl bg-white px-5 py-8">
        {items.map((todo) => (
          <TodoListUi.Item
            key={todo.id}
            todo={todo}
